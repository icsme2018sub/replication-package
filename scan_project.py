...
def get_test_and_classes(proj_path, save = True):
    ...
    - save: flag for saving on a file or no

    """
    name = os.path.basename(proj_path)

    loc = proj_path+'/pom.xml'  # pom location
    ...
    project = Project(name, include_pattern, exclude_pattern)
...
def csv_out(lst, project):
    ...
    """ 
    last_commit = get_last_commit_id(project.get_project_name())
    ...
    frame = pd.DataFrame(OrderedDict((('project', projects),
                                      ...
                                      ('path_src', path_src),
                                      ('class_name', src_name))))
    output = 'results/res_'+project.get_project_name()+'.csv'
