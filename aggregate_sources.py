...
def process_results(mutation='results.csv', smells='factors/test-smells.csv', ck='factors/ck-metrics.csv',
                    code_smells='factors/code-smells', readability='factors/readability', output='merge.csv'):
    """
    ...
    :param process: the directory that contains the process metrics (1 for each process)
    ...
    process_metrics = ['ADD', 'DEL', 'COMM', 'DDEV', 'SCTR']
    ...
    for metric in code_ck_metrics:
        ...
        filtered_frame[metric+"_prod"] = filtered_frame.apply(lambda x: get_ck_value(x, True, ck_frame, metric),
                                                              axis=1)
    ...
    for metric in code_ck_metrics:
        print("- Processing {}".format(metric))
        filtered_frame[metric+"_test"] = filtered_frame.apply(lambda x: get_ck_value(x, False, ck_frame, metric),
                                                              axis=1)
    ...
    prod_readability = pd.read_csv('factors/readability/source_readability.csv')
    test_readability = pd.read_csv('factors/readability/test_readability.csv')
    print("- Processing production readability")
    filtered_frame['prod_readability'] = filtered_frame.apply(lambda x: get_readability(x, prod_readability, True),
                                                              axis=1)
    print("- Processing test readability")
    filtered_frame['test_readability'] = filtered_frame.apply(lambda x: get_readability(x, test_readability, True),
                                                              axis=1)
...
def get_readability(row, frame, flag):
    """
    ...
    :param flag: flag for production or test
    """
    if flag:
        name = 'class_name'
    else:
...
def get_production_code_smell(row, frame, smell):
    """
...
def get_smell_value(row, smells_frame, smell):
    """
...
def get_ck_value(row, flag, ck_frame, metric):
    """
    Returns the value for the given metric for a particular source
    :param row: the data frame row for the test
    :param flag: true if that's a production class; false if a test
...
def separate_sets(complete_frame='merge.csv', delimiter='quartile'):
    """
    It separates
    ...
    if delimiter == 'quartile':
        ...
        bad_tests.to_csv('bad_tests.csv', index=False)
        good_tests.to_csv('good_tests.csv', index=False)
        ...
        bad_tests.to_csv('bad_tests_median.csv', index=False)
        good_tests.to_csv('good_tests_median.csv', index=False)
