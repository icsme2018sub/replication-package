...
def get_clone_script(project_list, dir='projects/', path='./get_projects.sh'):
    ...
    Arguments
    -------------
    - project_list: the pandas DataFrame with all the information about the projects
...
def get_script(project_list, path='./run_experiment.sh'):
    ...
    script.write('rm -rf mutation_results logs\n')
    script.write('mkdir mutation_results logs\n')

    for i, row in project_list.iterrows():
        name = row['project']
        script.write('echo \'* {} out of {} -> {}\'\n'.format(i+1, len(project_list), name))
        script.write('mkdir mutation_results/{}\n'.format(name))
        ...
        pairs = get_test_and_classes('projects/'+name)
...
def calculate_results():
    ...
    return "{} calculate_results.py".format(python_command)
...
def generate_sequence_for_each_project(project, script, pairs):
    ...
    for pair in pairs:
        ...
        python_command = 'python' if platform.system() == 'Darwin' else 'python3'
        script.write('\n{} ../../pom_changer.py {} {} {}\n'.format(python_command, project, class_to_mutate, test_to_run))
        ...
        script.write('cp -r target/{} ../../mutation_results/{}\n\n'.format(test_to_run, project))
...
def main():
    ...
    projects_list = pd.read_csv(projects_csv)
